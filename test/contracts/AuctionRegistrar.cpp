...
protected:
	...
	public:
		...
		h256 owner(string const& _name)
		{
			return callStringReturnsAddress("owner", _name);
		}
		void setAddress(string const& _name, h256 const& _address, bool _primary)
		{
			callStringAddressBool("setAddress", _name, _address, _primary);
		}
		h256 addr(string const& _name)
		{
			return callStringReturnsAddress("addr", _name);
		}
		string name(h256 const& _addr)
		{
			return callAddressReturnsString("name", _addr);
		}
		void setSubRegistrar(string const& _name, h256 const& _address)
		{
			callStringAddress("setSubRegistrar", _name, _address);
		}
		h256 subRegistrar(string const& _name)
		{
		...
		void transfer(string const& _name, h256 const& _target)
		{
...
BOOST_AUTO_TEST_CASE(reserve)
{
	...
	// should not work
	registrar.reserve("");
	BOOST_CHECK_EQUAL(registrar.owner(""), h256{});
...
BOOST_AUTO_TEST_CASE(properties)
{
	...
	for (string const& name: names)
	{
		...
		registrar.setAddress(name, h256(addr), true);
		BOOST_CHECK_EQUAL(registrar.addr(name), h256(addr));
		registrar.setSubRegistrar(name, h256(addr + 20));
		BOOST_CHECK_EQUAL(registrar.subRegistrar(name), h256(addr + 20));
		...
		BOOST_CHECK_EQUAL(registrar.owner(name), sender);
		registrar.setAddress(name, h256(addr + 1), true);
		BOOST_CHECK_EQUAL(registrar.addr(name), h256(addr));
		registrar.setSubRegistrar(name, h256(addr + 20 + 1));
		BOOST_CHECK_EQUAL(registrar.subRegistrar(name), h256(addr + 20));
...
BOOST_AUTO_TEST_CASE(transfer)
{
	...
	registrar.transfer(name, h256(555));
	BOOST_CHECK_EQUAL(registrar.owner(name), h256(555));
...
BOOST_AUTO_TEST_CASE(disown)
{
	...
	registrar.reserve(name);
	registrar.setContent(name, h256(u256(123)));
	registrar.setAddress(name, h256(124), true);
	registrar.setSubRegistrar(name, h256(125));
	BOOST_CHECK_EQUAL(registrar.name(h256(124)), name);
	...
	m_sender = account(0);
	registrar.disown(name);
	BOOST_CHECK_EQUAL(registrar.owner(name), h256());
	BOOST_CHECK_EQUAL(registrar.addr(name), h256());
	BOOST_CHECK_EQUAL(registrar.subRegistrar(name), h256());
	BOOST_CHECK_EQUAL(registrar.content(name), h256());
	BOOST_CHECK_EQUAL(registrar.name(h256(124)), "");
}
